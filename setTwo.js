...
const uniqArr = (param1) => `The new names array with all the duplicate items removed is ${param1}`
...
// console.log(uniq(names, uniqArr))
...
makeString = (x,y) => console.log(`The item at index ${x} is ${y}`)

each(names, makeString)
...
// Do not edit the code above.
// so we use id to check which one has it then we use that index to print the whole dictionary out

function getUserById (arr, id, callback) {
  for (let i = 0; i < arr.length; i++){
    if (arr[i].id === id) {
      // then we take this index and use it in our callback function
      callback(arr[i])
    } else{
      // console.log( "No matching id was found") 
    }
  }
}

const printThisObject = (indexOfObject) => `You did it: Here's the matching id: ${indexOfObject}`
